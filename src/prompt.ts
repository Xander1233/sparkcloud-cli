...
type QuestionsThatReturnAString<T extends inquirer.Answers> = inquirer.RawListQuestion<T> | inquirer.ExpandQuestion<T> | inquirer.InputQuestion<T> | inquirer.PasswordQuestion<T> | inquirer.EditorQuestion<T>;
...
export async function prompt(
	...
	if (prompts.length && options.nonInteractive) {
		...
		throw new SparkCloudError(`Missing required options (${missingOptions}) while running in non-interactive mode`, {
			children: prompts,
		});
	}
...
export async function promptOnce<A extends inquirer.Answers>(question: QuestionsThatReturnAString<A>, options?: Options): Promise<string>;
export async function promptOnce<A extends inquirer.Answers>(question: inquirer.CheckboxQuestion<A>, options?: Options): Promise<string[]>;
export async function promptOnce<A extends inquirer.Answers>(question: inquirer.ConfirmQuestion<A>, options?: Options): Promise<boolean>;
export async function promptOnce<A extends inquirer.Answers>(question: inquirer.NumberQuestion<A>, options?: Options): Promise<number>;
...
export async function promptOnce<A extends inquirer.Answers>(question: inquirer.ListQuestion<A>, options?: Options): Promise<any>;
 ...
 */
export async function confirm(args: { nonInteractive?: boolean; force?: boolean; default?: boolean; message?: string }): Promise<boolean> {
	if (!args.nonInteractive && !args.force) {
		...
		throw new SparkCloudError('Pass the --force flag to use this command in non-interactive mode');
	} else {
		return true;
	}
}
