...
import { logSuccess, isVSCodeExtension } from "../utils";
...
function getChildEnvironment(target: string, overallOptions: any, config: any) {
	// active project ID
	const projectId = overallOptions.projectId;
	...
	return Object.assign({}, process.env, {
	  SPARKCLOUD_PROJECT: projectId,
	  PROJECT_DIR: projectDir,
	  RESOURCE_DIR: resourceDir,
	});
}

function runTargetCommands(target: string, hook: string, options: any, config: any) {
	...
	const childOptions = {
		...
		env: getChildEnvironment(target, options, config),
		shell: true,
		stdio: [0, 1, 2]
	};
	...
	let logIdentifier = `${target}:${hook}`;
	if (config.target) {
		logIdentifier += ` (${config.target})`;
	}

	return runAllCommands.then(() => {
		logSuccess(clc.green(clc.bold(logIdentifier + ':')) + " Finished running " + clc.bold(hook) + ' script.');
	}).catch((err: any) => {
		throw new SparkCloudError(`${logIdentifier} ${hook} error: ${err.message}`);
	});
}

function getReleventConfigs(target: string, options: any) {
	...
	onlyTargets = onlyTargets.filter((individualOnly: string) => {
		return individualOnly.startsWith(`${target}:`);
	}).map((individualOnly: string) => {
		...
		for (const config of targetConfigs) {
		  if (!config.codebase) {
			...
			const found = onlyTargets.find(
			  (individualOnly: string) => config.codebase === individualOnly.split(":")[0]
			);
			if (found) {
			  onlyConfigs.push(config);
			  matched[found] = true;
...
export function lifecycleHooks(target: string, hook: string): (context: any, options: any) => Promise<any> {
	return function(context: any, options: any) {
		return getReleventConfigs(target, options).reduce((previousCommands: Promise<unknown>, individualConfig: any) => {
