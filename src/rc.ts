...
export function loadRC(options: { cwd?: string; [other: string]: any }) {
  ...
  const potential = path.resolve(dir || cwd, "./.sparkcloudrc");
...
export class RC {
  ...
  static loadFile(rcpath: string): RC {
    let data = {};
    if (utils.fileExistsSync(rcpath)) {
      try {
  ...
  constructor(rcpath?: string, data?: Partial<RCData>) {
    this.path = rcpath;
    this.data = { project: {}, targets: {}, etags: {}, ...data };
  }
  ...
  get hasProjects(): boolean {
    return this.data.project.name.length > 0;
  }
  ...
  sparkcloud target:apply ${type} ${name} <resources...>`
      );
    }
