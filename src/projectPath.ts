 ...
 */
export function resolveProjectPath(options: { cwd?: string; configPath?: string }, filePath: string): string {
	const projectRoot = detectProjectRoot(options);
	if (!projectRoot) {
		throw new SparkCloudError('Expected to be in a project directory, but none was found.', {
			exit: 2,
		});
	}
